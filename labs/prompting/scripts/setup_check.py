...
def check_python():
    ...
    # Check if using the recommended interpreter
    recommended = "/usr/local/bin/python"
    if sys.executable == recommended:
        print(f"   {C.G}✅ Using recommended Python interpreter{C.D}")
    else:
        print(f"   {C.Y}⚠️  Recommended: {recommended}{C.D}")
        print(f"   {C.Y}   Current: {sys.executable}{C.D}")
        print(f"   {C.Y}   This may cause import issues with pytest{C.D}")
    
    if sys.version_info >= (3, 8):
        print(f"   {C.G}✅ Python version is compatible{C.D}")
        return True
    else:
        print(f"   {C.R}❌ Python 3.8+ required{C.D}")
        return False
